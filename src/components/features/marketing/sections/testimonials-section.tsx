...
const servicePillars = [
  {
    ...
    description:
      "Đội ngũ kiểm tra chất lượng hình và bố cục trước khi duyệt đơn.",
  },
  {
    ...
    >
      <div className="page-shell">
        <div className="mb-8 flex items-end justify-between gap-4">
          ...
          <Badge
            variant="outline"
            className="hidden border-(--mirai-color-brand) bg-(--mirai-color-brand)/10 text-(--mirai-color-brand-strong) md:inline-flex"
          >
        ...
        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="grid gap-4 sm:grid-cols-2">
            {testimonials.map((t) => (
              ...
              >
                <Quote className="absolute right-4 top-4 h-5 w-5 text-(--mirai-color-brand-strong)/20" />
                <div className="mb-3 flex gap-1">
                  {Array.from({ length: t.rating }).map((_, index) => (
                    <Star
                      key={index}
                      className="h-3.5 w-3.5 fill-current text-(--mirai-sem-warning)"
                    />
                  ))}
                </div>
                ...
                <div className="mt-4 flex items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-(--mirai-color-brand)/15 px-2 py-1 text-[10px] font-medium text-(--mirai-color-brand-strong)">
                    {t.badge}
                  </span>
                </div>
              </article>
          ...
          <aside className="rounded-[10px] border border-border bg-card p-5">
            <h3 className="font-heading text-2xl font-semibold">
              MIRAI Service Promise
            </h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Trải nghiệm mua sắm rõ ràng, minh bạch và phù hợp cho flow
              ecommerce.
            </p>
            ...
            <Button
              asChild
              className="mt-6 h-11 w-full rounded-[6px] bg-(--mirai-color-brand-strong) text-(--mirai-sem-background) hover:opacity-90"
            >
              <Link href="/customize">
                Thiết kế ngay
